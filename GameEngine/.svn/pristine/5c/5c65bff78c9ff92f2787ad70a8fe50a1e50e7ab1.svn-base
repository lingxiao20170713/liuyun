...
public class HUDText : MonoBehaviour
{
	protected class Entry
	{
		...
		public Text label;		    // Label on the game object
	...
	[HideInInspector][SerializeField] Font font;
	...
	Entry Create ()
	{
        ...
        ne.label = new GameObject().AddComponent<Text>();
        ne.label.transform.parent = GameObject.Find("GUI/OverLayer").transform;
		ne.label.name = counter.ToString();
		ne.label.font = (Font)ambigiousFont;
		...
		ne.label.color = effectColor;
		ne.label.resizeTextForBestFit = true;
	...
	public void Add (object obj, Color c, float stayDuration)
	{
		...
		ne.label.color = c;
        ne.label.CrossFadeAlpha(0f, 0f, false);
		ne.val = val;
	...
	void Update ()
	{
		...
		for (int i = mList.Count; i > 0; )
		{
            ...
            ent.label.CrossFadeAlpha(a,0f,false);

			// Make the label scale in
			float s = scaleCurve.Evaluate(time - ent.time);
			if (s < 0.001f) s = 0.001f;
			ent.label.transform.localScale = new Vector3(s, s, s);
		...
		for (int i = mList.Count; i > 0; )
		{
			...
			ent.label.transform.localPosition = new Vector3(0f, offset, 0f);
			offset += Mathf.Round(ent.label.transform.localScale.y * ent.label.fontSize);
		}
	}
}
